# Vaidyasala — Claude Code Prompt Pack

**Phase-by-phase prompts for building the entire project with Claude Code in VS Code.**
Designed for: minimum token usage · 100% autopilot (zero questions) · resumable across sessions.

---

# PART 1 — ONE-TIME SETUP (do this before any phase)

### Step 1. Create the project folder and put two files in it

```
<LOCAL_PROJECT_FOLDER>/
├── docs/ARCHITECTURE.md      ← the approved architecture document (already delivered)
├── CLAUDE.md                 ← copy from Part 2 below (the Universal Laws)
└── docs/VARIABLES.md         ← copy from Part 3 below, fill in your details
```

### Step 2. Why this structure saves tokens

Claude Code automatically reads `CLAUDE.md` at session start — so the laws are enforced in **every** session without repeating them in prompts. `docs/PROGRESS.md` (created by the kickoff prompt) carries state between sessions, so each new session reads a 30-line status file instead of re-exploring the whole repo. Each phase prompt below tells Claude exactly **which architecture sections to read** — never the whole document.

### Step 3. Run phases in order

Open Claude Code in VS Code (`<LOCAL_PROJECT_FOLDER>` as workspace), paste one phase prompt, let it finish, verify the exit checks pass, start a **fresh session** for the next phase (fresh sessions = small context = fewer tokens).

---

# PART 2 — CLAUDE.md (THE UNIVERSAL LAWS)

> Save this exactly as `CLAUDE.md` in the project root. It applies to every session automatically.
...
# PART 3 — docs/VARIABLES.md (fill in, keep local)

> Save as `docs/VARIABLES.md`. Fill the `<>` placeholders when you have them — phases 0–6 run fully locally, so you can start TODAY with only the LOCAL and GITHUB sections filled. VPS/domain are only needed at Phase 7. **Real secrets go in `.env` files, never in this doc — this doc holds names/hosts/IDs only.**
...
## GITHUB
GITHUB_REPO = <e.g. github.com/<you>/vaidyasala>
DEFAULT_BRANCH = main
REGISTRY = ghcr.io/<you>/vaidyasala
...
# PART 5 — MAINTENANCE PROMPTS (reusable any time)

**Resume after any interruption** (paste in a fresh session):

```text
Follow CLAUDE.md laws. Read docs/PROGRESS.md and continue from "next step" to the
end of the current phase. Zero questions. Standard terse report.
```

**Fix CI failure:**

```text
Follow CLAUDE.md laws. CI failed on <link/paste job name>. Read only the failing
log + implicated files. Fix, push, confirm green. Terse report.
```

**Regenerate AI content after a model upgrade:**

```text
Follow CLAUDE.md laws. Bump modelVersion in packages/core/ai config to <version>.
Queue regeneration (steps: correct, translate, enrich, article, embed) for all
PUBLISHED videos. Confirm EnrichmentDiff visible in admin. Terse report.
```

---

# PART 6 — TOKEN-SAVING RULES FOR *YOU* (the human)

1. **One phase = one fresh session.** Never continue a finished phase's session — old context is dead weight you pay for on every message.
2. **Never paste the architecture doc into chat.** It lives at docs/ARCHITECTURE.md; prompts already point Claude at the exact §sections.
3. **Don't ask "explain what you did"** — read the commits and PROGRESS.md instead; explanation turns are pure token spend.
4. **If a phase fails an exit check,** use the CI-fix or Resume prompt in a fresh session rather than a long back-and-forth in the old one.
5. **Trust the DECISIONS.md log.** Autopilot decisions are recorded there; review it weekly instead of interrogating Claude in-session.
6. **Run `/compact` if a session must go long,** and keep VS Code diff review (not chat) as your code-review surface.
7. Phases 0–6 need **no VPS, no domain** — everything runs on your PC in `C:\web\Vaidyasala`. Missing keys don't stop the build (LAW 1 stubs them against fixtures); add keys later and re-run the live smoke tests.
8. **Shared-VPS discipline:** never skip 7-PRE, and never let a session touch the server outside a Phase 7 prompt. The #1 exit check of every server session is that your 10 existing sites show unchanged container uptime. If 7-PRE reports NO-GO, that's the system working — review SERVER-AUDIT.md before forcing anything.
```

---

*End of prompt pack. Fill docs/VARIABLES.md, save CLAUDE.md, run Phase 0.*
